    ...
    >
      <section className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20">
        <div className="grid gap-6 lg:grid-cols-3">
          {dxPacks.map((pack) => (
            <article key={pack.slug} className="flex h-full flex-col rounded-2xl border border-line-soft p-6 shadow-sm">
              <span className="w-fit rounded-full bg-cat-cream px-3 py-1 text-xs font-medium text-navy">{serviceStatusLabels[pack.status]}</span>
              <h2 className="mt-5 text-xl font-medium">{pack.name}</h2>
              <p className="mt-3 text-sm font-semibold leading-6 text-sky-900">{pack.catchphrase}</p>
              ...
              <Link href={`/services/dx/packs/${pack.slug}`} className="mt-auto inline-flex min-h-12 items-center gap-2 pt-6 font-semibold text-sky-900">
                詳細を見る
