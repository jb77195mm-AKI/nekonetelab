    ...
    <div className="bg-white text-ink">
      ...
      <section className="relative overflow-hidden bg-[linear-gradient(135deg,#eef6fa_0%,#ffffff_52%,#faf6ee_100%)]">
        ...
        <div className="relative mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 sm:py-24 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full border border-sky-200 bg-white px-4 py-2 text-xs font-medium text-sky-950">
              <Layers3 className="h-4 w-4" aria-hidden="true" />
              小規模事業者向け・業種別DX
            </p>
            <h1 className="mt-6 text-[clamp(2.5rem,9vw,4.6rem)] font-medium leading-[1.08] tracking-tight">
              今の業務を変えすぎず、
            ...
            <p className="mt-4 max-w-3xl text-sm font-semibold leading-7 text-ink">
              最初から大規模なシステムを導入せず、効果が確認しやすい業務から小さく始められます。
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              <Link href="/contact?plan=dx-consultation" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-sky-900 px-7 py-3 font-semibold text-white">
                無料で相談する
                <ArrowRight className="h-4 w-4" aria-hidden="true" />
              </Link>
              <a href="#packs" className="inline-flex min-h-12 items-center justify-center rounded-full border-2 border-slate-800 bg-white px-7 py-3 font-semibold">
                3つのDXパックを見る
              </a>
              <Link href="/services/dx/tools/inbound-queue" className="inline-flex min-h-12 items-center justify-center rounded-full border-2 border-sky-800 bg-white px-7 py-3 font-semibold text-sky-900">
                多言語受付のデモを見る
              </Link>
            </div>
          </div>
          <div className="rounded-[2rem] border border-white bg-white/95 p-6 shadow-sm shadow-slate-900/10 sm:p-8">
            <p className="text-xs font-medium tracking-[0.16em] text-navy">CORE MESSAGE</p>
            <h2 className="mt-3 text-2xl font-medium">すべてを入れ替えないDX</h2>
            <div className="mt-6 space-y-3">
              {[
                ...
                <div key={step} className="grid grid-cols-[2.5rem_1fr] gap-3 rounded-2xl bg-cream-light p-4">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full bg-sky-900 text-xs font-medium text-white">{step}</span>
                  <div>
      ...
      <section className="border-y border-line-soft bg-ink py-20 text-white sm:py-24">
      ...
      <section className="py-20 sm:py-24">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-3">
            {dxTools.map((tool) => (
              <article key={tool.slug} className="flex h-full flex-col rounded-2xl border border-line-soft bg-white p-6 shadow-sm">
                ...
                <p className="mt-5 text-xs font-medium tracking-[0.12em] text-navy">{tool.category}</p>
                <h3 className="mt-2 text-xl font-medium">{tool.name}</h3>
                ...
                <div className="mt-5 rounded-2xl bg-cream-light p-4 text-sm font-semibold">
                  初期 {formatPriceRange(tool.initialPrice)}
                  <br />
                  月額 {formatPriceRange(tool.monthlyPrice)}
                </div>
                <Link href={`/services/dx/tools/${tool.slug}`} className="mt-auto inline-flex min-h-12 items-center gap-2 pt-6 font-semibold text-sky-900 underline-offset-4 hover:underline">
                  標準範囲とオプションを見る
                  <ArrowRight className="h-4 w-4" aria-hidden="true" />
                </Link>
              </article>
      ...
      <section id="packs" className="scroll-mt-24 py-20 sm:py-24">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          />
          <div className="mt-10 hidden overflow-x-auto rounded-2xl border border-line-soft md:block">
            <table className="min-w-[52rem] w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-line-soft">
                {dxPackComparisonRows.map((row) => (
                  <tr key={row.label}>
                    <th scope="row" className="bg-cream-light p-4 font-medium">{row.label}</th>
                    {row.values.map((value, index) => <td key={dxPacks[index].slug} className="p-4 leading-6 text-muted">{value}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="mt-10 grid gap-5 md:hidden">
            {dxPacks.map((pack, packIndex) => (
              <article key={pack.slug} className="rounded-2xl border border-line-soft p-5">
                ...
                <dl className="mt-4 divide-y divide-line-soft">
                  {dxPackComparisonRows.map((row) => (
                    <div key={row.label} className="grid grid-cols-[6.5rem_1fr] gap-3 py-3 text-sm">
                      ...
                      <dd className="leading-6 text-muted">{row.values[packIndex]}</dd>
                    </div>
                  ))}
                </dl>
              </article>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-3">
            {[["70%", "共通基盤", "アカウント・タスク・承認・通知・履歴・レポート"], ["20%", "業種別テンプレート", "再来店・案件管理・在庫発注などの業種別設計"], ["10%", "お客様別設定", "ルール・権限・項目・既存サービスへの合わせ込み"]].map(([ratio, title, text]) => (
              <article key={title} className="rounded-2xl border border-line-soft bg-white p-6">
                <p className="text-4xl font-medium text-sky-900">{ratio}</p>
                <h3 className="mt-3 text-lg font-medium">{title}</h3>
                <p className="mt-3 text-sm leading-7 text-muted">{text}</p>
              </article>
            ))}
          </div>
          <p className="mt-4 text-xs leading-6 text-muted">※ 割合は設計方針の目安です。業種・規模・既存システムによって変わります。</p>
        </div>
      </section>
      ...
      <section className="py-20 sm:py-24">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <div className="mt-6 rounded-2xl border border-line-soft p-6">
            <h3 className="font-medium">安全な運用のために</h3>
            ...
            <p className="mt-4 text-xs leading-6 text-muted">未実装の機能は、導入内容に応じて設計します。</p>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <ol className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {dxIntroductionFlow.map(([title, description], index) => (
              <li key={title} className="rounded-2xl border border-sky-800 bg-sky-900 p-6">
                <span className="text-xs font-medium tracking-[0.16em] text-amber-soft">STEP {index + 1}</span>
                <h3 className="mt-3 text-lg font-medium">{title}</h3>
                <p className="mt-3 text-sm leading-7 text-sky-100">{description}</p>
              </li>
            ))}
          </ol>
          <p className="mt-6 rounded-2xl bg-sky-900 p-5 text-sm font-semibold leading-7 text-sky-100">
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {industryTemplates.map((industry) => (
              <article key={industry.name} className="flex h-full flex-col rounded-2xl border border-line-soft p-5">
                <h3 className="text-lg font-medium">{industry.name}</h3>
                <p className="mt-3 text-sm leading-7 text-muted">{industry.problems}</p>
                <ul className="mt-4 flex flex-wrap gap-2">
                  {industry.features.map((feature) => <li key={feature} className="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-semibold text-sky-950">{feature}</li>)}
                </ul>
                <p className="mt-5 text-xs font-medium text-navy">関連：{industry.relatedPack}</p>
                <Link href={industry.relatedPackSlug ? `/services/dx/packs/${industry.relatedPackSlug}` : `/contact?plan=dx-consultation`} className="mt-auto inline-flex min-h-11 items-center gap-2 pt-4 text-sm font-semibold text-sky-900 underline-offset-4 hover:underline">
                  相談・詳細を見る
                  <ArrowRight className="h-4 w-4" aria-hidden="true" />
                </Link>
              </article>
      ...
      <section className="border-y border-line-soft bg-cream-light/60 py-20 sm:py-24">
        <div className="mx-auto grid max-w-6xl gap-6 px-4 sm:px-6 lg:grid-cols-2">
          <div className="rounded-2xl bg-white p-6 sm:p-8">
            <h2 className="text-2xl font-medium">標準サポート</h2>
            <ul className="mt-5 space-y-3">
              {supportRules.included.map((item) => <li key={item} className="flex items-start gap-2 text-sm leading-6 text-muted"><Check className="mt-1 h-4 w-4 shrink-0 text-emerald-700" aria-hidden="true" />{item}</li>)}
            </ul>
          </div>
          <div className="rounded-2xl bg-white p-6 sm:p-8">
            <h2 className="text-2xl font-medium">正式な受付と対応時間</h2>
            <p className="mt-4 text-sm leading-7 text-muted">正式な依頼は専用フォームまたはメールで受け付けます。</p>
            <dl className="mt-5 grid gap-3 text-sm">
              {[supportRules.businessHours, supportRules.holidays, supportRules.normalReply, supportRules.normalChange, supportRules.urgent].map((item) => <div key={item} className="rounded-xl bg-cream-light px-4 py-3 font-semibold">{item}</div>)}
            </dl>
            <p className="mt-5 text-xs leading-6 text-muted">{supportRules.notice}</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-ink py-20 text-white sm:py-24">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6">
          <p className="text-xs font-medium tracking-[0.18em] text-amber-soft">FREE CONSULTATION</p>
          <h2 className="mt-4 text-3xl font-medium sm:text-4xl">今の業務と既存ツールから、一緒に整理します。</h2>
          ...
          <Link href="/contact?plan=dx-consultation" className="mt-8 inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-white px-7 py-3 font-semibold text-ink">
            無料相談フォームへ
            <ArrowRight className="h-4 w-4" aria-hidden="true" />
          </Link>
        </div>
...
function SectionTitle({ eyebrow, title, description, inverted = false }: { eyebrow: string; title: string; description?: string; inverted?: boolean }) {
  return (
    <div className="max-w-3xl">
      <p className={`text-xs font-medium tracking-[0.18em] ${inverted ? "text-amber-soft" : "text-navy"}`}>{eyebrow}</p>
      <h2 className={`mt-3 text-[clamp(2rem,7vw,3rem)] font-medium leading-tight ${inverted ? "text-white" : "text-ink"}`}>{title}</h2>
      {description ? <p className={`mt-4 text-sm leading-7 sm:text-base ${inverted ? "text-slate-300" : "text-muted"}`}>{description}</p> : null}
    </div>
  );
}
  ...
  return (
    <article className="rounded-2xl border border-slate-700 bg-slate-900 p-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-medium text-amber-soft">LAYER {step}</span>
      </div>
      <h3 className="mt-5 text-xl font-medium">{title}</h3>
      <ul className="mt-5 flex flex-wrap gap-2">
        {items.map((item) => <li key={item} className="rounded-full border border-slate-700 bg-ink px-3 py-1.5 text-xs font-semibold text-slate-300">{item}</li>)}
      </ul>
    </article>
  );
}
...
function RoleCard({ icon: Icon, title, items, tone }: { icon: typeof Bot; title: string; items: string[]; tone: "sky" | "orange" }) {
  const styles = tone === "sky" ? "border-sky-200 bg-sky-50 text-sky-900" : "border-cat-beige bg-cream-light text-navy";
  return (
    <article className={`rounded-2xl border p-6 sm:p-8 ${styles}`}>
      ...
      <ul className="mt-5 grid gap-3 sm:grid-cols-2">
        {items.map((item) => <li key={item} className="rounded-xl bg-white px-4 py-3 text-sm font-semibold">{item}</li>)}
      </ul>
    </article>
  );
}
