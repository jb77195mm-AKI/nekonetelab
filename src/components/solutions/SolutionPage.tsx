    ...
    <main>
      <section className="relative overflow-hidden bg-gradient-to-br from-sky-50 via-white to-cream-light">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-24">
          <Link
            href="/solutions"
            className="inline-flex min-h-11 items-center gap-2 rounded-full text-sm font-bold text-sky-900 underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-sky-800"
          >
          ...
          <div className="mt-7 grid gap-10 lg:grid-cols-[1.08fr_0.92fr] lg:items-center">
            <div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold tracking-[0.08em] text-sky-900">{solution.label}</p>
                  <p className="mt-1 font-black text-slate-950">{solution.name}</p>
                </div>
              </div>
              <h1 className="mt-6 text-[clamp(2.25rem,8vw,4rem)] font-black leading-tight tracking-tight text-slate-950">
                {solution.heroTitle}
              </h1>
              <p className="mt-6 max-w-3xl text-base leading-8 text-slate-700 sm:text-lg">{solution.description}</p>
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {solution.cardFeatures.map((feature, index) => (
              <article key={feature} className="rounded-2xl border border-slate-700 bg-slate-900 p-6">
                <span className="flex h-9 w-9 items-center justify-center rounded-full bg-sky-800 text-sm font-black">
                  {index + 1}
                </span>
                <h2 className="mt-4 text-lg font-black">{feature}</h2>
        ...
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr]">
          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {solution.features.map((feature) => (
              <li key={feature} className="flex items-start gap-3 rounded-2xl bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-700">
          ...
          <ol className="mt-10 grid gap-5 md:grid-cols-3">
            {solution.usage.map((step, index) => (
              <li key={step.title} className="rounded-3xl border border-slate-200 p-6">
                <span className="text-xs font-black tracking-[0.18em] text-sky-900">STEP {index + 1}</span>
          ...
          <div className="mx-auto grid max-w-6xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            ...
            <div className="rounded-3xl border border-cat-beige bg-white p-7 shadow-lg shadow-navy-deep/5 sm:p-9">
              ...
              {solution.pricing.upperPlan ? (
                <p className="mt-5 rounded-2xl bg-sky-50 px-4 py-3 text-sm font-bold text-sky-950">
                  {solution.pricing.upperPlan}
                </p>
              ) : null}
              <p className="mt-5 text-xs leading-6 text-slate-600">※ {solution.pricing.note}</p>
            </div>
          </div>
        </section>
      )}
        ...
        <ol className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {solutionFlow.map((step, index) => (
            <li key={step} className="rounded-2xl border border-slate-200 p-5">
              <span className="flex h-8 w-8 items-center justify-center rounded-full bg-sky-900 text-xs font-black text-white">
                {index + 1}
              </span>
      ...
      <section className="bg-sky-950 py-20 text-white sm:py-24">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6">
          <p className="text-xs font-bold tracking-[0.18em] text-sky-200">CONTACT</p>
          <h2 className="mt-4 text-3xl font-black sm:text-4xl">今の業務に合う形を、一緒に整理します。</h2>
          <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-sky-100">
            このページはサービス検証用のデモです。導入可否や正式な仕様は、現在の業務を伺ったうえでご案内します。
          </p>
          <Link
            href="/#contact"
            className="mt-8 inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-white px-7 py-3 font-bold text-sky-950 transition hover:bg-sky-100 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white"
          >
...
function SectionHeading({
  ...
  return (
    <div className="max-w-2xl">
      <p className={`text-xs font-bold tracking-[0.18em] ${inverted ? "text-sky-300" : "text-sky-900"}`}>
        {eyebrow}
      </p>
      <h2 className={`mt-3 text-3xl font-black sm:text-4xl ${inverted ? "text-white" : "text-slate-950"}`}>
        {title}
      </h2>
      {description ? (
